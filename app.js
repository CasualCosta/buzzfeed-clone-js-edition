...
const populateQuestions = () => {
    questions.forEach(question => {
        ...
        unansweredQuestionsId.push(question.id)
...
const handleClick = (questionId, chosenAnswer) => {
    if(unansweredQuestionsId.includes(questionId))
        chosenAnswers.push(chosenAnswer)
    const itemToRemove = unansweredQuestionsId.indexOf(questionId)

    if(itemToRemove > -1) {
        unansweredQuestionsId.splice(itemToRemove, 1)
    }
    console.log(`Chosen Answers: ${chosenAnswers} Unanswered questions: ${unansweredQuestionsId}.`)

    const lowestQuestionId = Math.min(...unansweredQuestionsId)
    location.href = `#${lowestQuestionId}`

    disableQuestionBlock(questionId, chosenAnswer)

    if(!unansweredQuestionsId.length) {
        showAnswer()
    }
}

const showAnswer = () => {
    console.log("All questions have been answered. Show answer now.")
    let result
